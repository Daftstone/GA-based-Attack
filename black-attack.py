...
from GA import GA
from utils import mnist_model, fmnist_model, cifar10_model, train
...
def attack():
    ...
    model = Model.model
    m_temp = {"mnist": mnist_model, "fmnist": fmnist_model, "cifar10": cifar10_model}
    Model_temp = m_temp[FLAGS.dataset]()
    subtitute_model = train(Model_temp, x_train, y_train, x_test, y_test, FLAGS.batch_size, FLAGS.nb_epochs,
                            FLAGS.sub_is_train)
    ...
    acc = model_eval(sess, x, y, model(x), x_test, y_test, batch_size=FLAGS.batch_size)
    print('Test legitimate examples of the target model: %.4f' % acc)
    acc = model_eval(sess, x, y, subtitute_model(x), x_test, y_test, batch_size=FLAGS.batch_size)
    print('Test legitimate examples of the substitute model: %.4f' % acc)
...
if __name__ == "__main__":
    ...
    flags.DEFINE_bool("sub_is_train", False, "traing online or load weight from file")
